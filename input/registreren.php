...
function wijzigen (){
	...
	echo '"></tr>	<tr>	<th>Naam</th>	<td>';
		...
		echo '<input type="text" name="email" size="40" required value="';
		echo $useremail;
		...
		echo '<tr><td colspan=2><input type="button" value="Wijzigen" onclick="reghash(this.form, this.form.oldpassword, this.form.newpassword)" /></td></tr>';
		echo '	</table>
					</table>
						</form>';
...
if (in_array($username, $admin)){
	...
	if (isset($_GET['changeusr'])){
		...
		//users();
		echo '<input type="button" value="Change your account" onclick=location.href="index.php?page=registreren&changeusr=yes">';
		toevoegen();
	...
	if (isset($_GET['changeusr'])){
		...
		echo '<input type="button" value="Change your account" onclick="index.php?page=registreren&changeusr=yes">';
		toevoegen();
