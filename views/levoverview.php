...
$selectbesch = "SELECT product.proname, beschikbaarheid.*, leverancier.levname, inkoper.inkname, landen.*
				...
				AND leverancier.levname = '".$_GET['lev']."'
...
<tr>
<th colspan="50">
			<h1>Leveranciers overzicht van '.$_GET['lev'].'</h1>
		</th>	</tr>
<th id="change"></th>
...
while ($row = mysql_fetch_array($resultbesch)) {
	...
	echo '</td><td id="product"><a href="index.php?page=prooverview&pro='.$row['proname'].'">'.$row['proname'].'</a></td>';	
	...
	foreach ($month as $k => $v){
		if ($row['kwar'.$v] == 'on') {	#product available
			...
			#counter for time part range
				$nr = 1;
				$leverancier[$supcount] = $row['proname'];
